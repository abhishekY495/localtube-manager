...
function createPlaylistElement(playlist: YoutubePlaylist): HTMLElement {
  ...
  playlistDiv.innerHTML = `
    <div class="youtube-playlist-container-1">
      ...
      <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" height="12" viewBox="0 0 12 12" width="12" focusable="false" aria-hidden="true" style="pointer-events: none; display: inherit; width: 100%; height: 100%;"><path d="M1 3h10v1H1V3Zm0 2h6v1H1V5Zm0 2h6v1H1V7Zm7-2 4 2.5L8 10V5Z"></path></svg>${playlist?.videosCount}</span>
    </div>
    <div class="youtube-playlist-container-2">
      <a href="https://www.youtube.com/playlist?list=${playlist?.urlSlug}" class="youtube-playlist-name" title="${playlist?.name}">${playlist?.name}
      </a>
      <p class="youtube-playlist-channelname" title="${playlist?.channelName}">@${playlist?.channelName?.trim()}</p>
    </div>
...
function renderBatch(
  ...
  const existingSentinel = playlistsContainer.querySelector(".youtube-playlists-sentinel");
  if (existingSentinel) {
  ...
  batch.forEach((playlist, batchIndex) => {
    ...
    const removeBtn = playlistElement.querySelector(".remove-youtube-playlist-btn");
